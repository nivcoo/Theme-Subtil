            ...
            <div class="col-md-3 shop-profil">
                <div class="card">
                                ...
                                <div class="dropdown btn-block">
                                    ...
                                    <ul class="dropdown-menu" style="padding: 0px;">
                                        <?php foreach ($search_categories_section[$v['Section']['id']] as $va) { ?>
                                            <li style="font-size: 14px;text-align: center;height: 40px;border: 1px solid #000;">
                                                <a style="color: #000;text-decoration: none;display: block;height: 40px;padding: 8px;"
                    ...
                    <div class="card-footer">
                        <div class="panel-heading text-center"><h3><b>Compte</b></h3></div>
                        <hr>
                        <?php if ($isConnected) { ?>
                            ...
                            <div class="col-sm-<?= $col ?> col-lg-<?= $col ?> col-md-<?= $col ?>">
                                <div class="thumbnail">
                                    <div class="caption" style="height:auto;width:auto;">
                                        <h4 style="color: black; text-align: center; font-family: 'Francois One', sans-serif;">
                                            <i
                                                    class="fa fa-angle-down"></i> <?= before_display($v['Item']['name']) ?>
                                            <i
                                                    class="fa fa-angle-down"></i></h4>
                                    </div>
                                    <?php if (isset($v['Item']['img_url'])) { ?><img class="w-100"
                                                                                     src="<?= $v['Item']['img_url'] ?>"
                                                                                     alt=""><?php } ?>
                                    <?php if (('CAN_BUY')) { ?>
                                        <button style="border-radius: 0px;"
                                                class="btn items btn-block pull-right display-item"
                                                data-item-id="<?= $v['Item']['id'] ?>"><?= $Lang->get('SHOP__BUY') ?>
                                            - <?= $v['Item']['price'] ?><?php if ($v['Item']['price'] == 1) {
                                                echo ' ' . $singular_money;
                                            } else {
                                                echo ' ' . $plural_money;
                                            } ?></button>
